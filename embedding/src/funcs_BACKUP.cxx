...
namespace Analysis {
  ...
  void AddCorrected2DHistograms( TH2D *h_ChgUE2D_corr[nEffEtaBins], TH2D *h_AddedChgUE2D_corr[nEtaBins] ) {

    for ( int f=0; f<nEffEtaBins; ++f ) {

      double tenEbin = -10.0 + f;
      
      int eval = 99;
      for (int e=0; e<nEtaBins; ++e) {    if (tenEbin>=(10*etaLo[e]) ) {eval = e;}    }
      if (eval==99) { std::cerr<<"error finding eta bin of histogram!   "<<tenEbin/10<<std::endl; continue; }

      h_AddedChgUE2D_corr[eval]->Add(h_ChgUE2D_corr[f]);
      // std::cout<<f<<": "<<h_ChgUE2D_corr[f]->GetName()<<" "<<emw[eval]<<std::endl;
    }
  ...
  void GenerateWeightedPtResponse( TH1D *h_DetWt[nPtBins], TH1D *h_Det[21], TH1D *h_MissedJets, TString plot_dir ) {
	...
	if ( ptVal >= ptLo[p] && ptVal <= ptHi[p] ) {
      ...
      // std::cout<<"Integral: "<<h_DetWt[p]->Integral()<<std::endl;
    }
    can5->BuildLegend(0.4,0.68,0.9,0.9);
    name = plot_dir; name += "WeightedPtResponse.pdf";
    can5->SaveAs(name,"PDF");
  ...
  void ProjectAndSaveFinalUEPlots( TH1D *h_WtUEpt, TString suffix, int eval, TString plot_dir ){
    ...
    h_WtUEpt->SetStats(1);
    h_WtUEpt->SetAxisRange( 0.0000001,10,"Y");
    ...
    text = "#LT#frac{dN_{ch}}{d#eta d#phi}#GT = "; text += h_WtUEpt->Integral()/area[eval];
    text = text(0,42);
  ...
  void ProjectAndScaleUEHistogramForAllPt( TH2D *h_ChgUE2D, TH1D *h_leadPt, TH1D *h_UEpt[55], int eval, TString dir_name ){
    ...
    for (int i=0; i<55; ++i) {  // det-level fractional pT contribution to part-level pT --> fc(pT_det)
      ...
      TString name = "hUEpt_"; name += etaBinName[eval]; name += "UE_"; name += ptVal; name += "GeV";
	...
	h_UEpt[i]->SetStats(0);
	name = emw[eval]; name += " UE, "; name += ptVal; name += " GeV det. jet";
	h_UEpt[i]->SetNameTitle(name,";chg. UE particle p_{T} (GeV)");
      }
    }
    can6->BuildLegend(0.68,0.1,0.9,0.9);
    TString saveName = dir_name + "UEptByLeadPt" + etaBinName[eval] + ".pdf";
    can6->SaveAs(saveName,"PDF");

    h_leadPt->Draw();
    saveName = dir_name + "leadPt.pdf";
    can6->SaveAs(saveName,"PDF");
  ...
  void ProjectPartLevelJetPt( TH2D* h_PtResponse, TH1D *h_Det[21], TString plot_dir ) {
    ...
    for (int i=0; i<21; ++i) {  // PROJECT FOR ALL PART-LEVEL BINS 10-30 GeV
      ...
      // std::cout<<ptVal<<"  "<<h_Det[i]->GetBinCenter(h_Det[i]->FindBin(ptVal))<<std::endl;
    }
  ...
  TH2D *ProjectUEHistograms( TH3D *h_ChgUE3D, int etaBin, TString plot_dir ) {
    ...
    TString name = "can0_"; name += etaBin;
    TCanvas *can0 = new TCanvas( name , "" ,700 ,500 );              // CANVAS 0
    can0->SetLogz();

    double eLo = -1.0 + (0.1*etaBin);
    double eHi = eLo + 0.1;
    
    h_ChgUE3D->GetZaxis()->SetRangeUser(eLo,eHi);
    ...
    name = "h_ChgUE2D_eta"; name += etaBin;
    h_ChgUE2D->SetName(name);
    h_ChgUE2D->Draw("COLZ");
    TString saveName = plot_dir + "ChgUE2D_eta"; saveName += etaBin; saveName += ".pdf";
    can0->SaveAs(saveName,"PDF");

    can0->Destructor();

    // std::cout<<eLo<<"  "<<eHi<<"  "<<name<<std::endl;
    
    return h_ChgUE2D;
  }

  
  void TrackingEfficiency2DCorrection( TH2D* h_ChgUE2D_te, TH2D* h_ChgUE2D_uncorr, TFile *efficFile, TString efficName ){
    
    double pt, effic, ptVal, corrVal, corrErr, jetPt; // eta, phi, e, px, py, pz, 
    int ptBin;

    TH1D *h_Effic = (TH1D*)efficFile->Get( efficName );
	...
	ptVal = h_ChgUE2D_te->GetYaxis()->GetBinCenter(iy);
	...
	ptBin = h_ChgUE2D_te->GetYaxis()->FindBin( ptVal );
    ...
    TString name = "can_"; name += efficName;
    TCanvas *can00 = new TCanvas( name , "" ,700 ,500 );              // CANVAS 00
    can00->SetLogz();
    h_ChgUE2D_te->GetYaxis()->SetRangeUser(0.0,15.0);
    h_ChgUE2D_te->Draw("COLZ");
    name = "plots/test/hiEA/"; name += h_ChgUE2D_te->GetName(); name += ".pdf";
    can00->SaveAs( name, "PDF");
    can00->Destructor();
    h_ChgUE2D_te->GetYaxis()->SetRangeUser(0.0,30.0);
  }
  ...
  void WeightUEPtByLeadPtAndFakes( TH1D *h_WtUEpt[nPtBins], TH1D *h_UEpt[55], TH1D *h_DetWt[nPtBins], TH1D *h_leadPt,
				   TH1D *h_FakeJets, int eval, TString plot_dir ){
    ...
    for (int p=0; p<nPtBins; ++p) {
      ...
      h_WtUEpt[p]->SetMarkerColor(etaColor[eval]);
      h_WtUEpt[p]->SetStats(0);
      h_WtUEpt[p]->Draw("SAME");
      std::cout<<h_WtUEpt[p]->Integral()/area[eval]<<std::endl;
      // std::cout<<h_WtUEpt[p]->Integral()<<"  "<<h_WtUEpt[p]->GetEntries()<<std::endl;
    }
    can7->SetLogy();
    can7->BuildLegend(0.4,0.68,0.9,0.9);
    TString saveName = plot_dir + "weightedUEptByLeadPt" + etaBinName[eval] + ".pdf";

...
int main () {
  ...
  TH2D *hEffic[nEffEtaBins];
  TH2D *hUE2D_te[55][nEtaBins];
  TH2D *hAddedUE_te[nPtBins][nEtaBins];

  for (int e=0; e<nEtaBins; ++e) {
    for (int p=0; p<nPtBins; ++p) {
      name = "Added_" + emw[e] + "UE_te" + ptBinName[p];
      hAddedUE_te[p][e] = new TH2D(name,";te corrected chg. UE part. p_{T} (GeV)",15,0.0,15.0);
    }
  }
  ...
  for (int e=0; e<nEtaBins; ++e) { // Z=UEeta
    ...
    for ( int jp=0; jp<55; ++jp ) {
      ...
      name = EAstring; name += "_UE2D"; name += etaBinName[e] + "_"; name += ptVal; name += "GeVdetJet";
      ...
      name = EAstring; name += "_UE2D_TRACKEFFIC"; name += etaBinName[e] + "_"; name += ptVal; name += "GeVdetJet";      
      hUE2D_te[jp][e] = new TH2D(name,";te corrected chg. UE part. p_{T} (GeV);chg. UE part. #eta",15,0.0,15.0,10,-1.0,1.0);
  ...
  // 	double pt = hUE2D[jp][e]->GetBinCenter(xbin);
  // 	if ( pt > 3.0 ) { pt = 3.0; }
  // 	double ptbin = hEffic[e]->FindBin(pt);
  // 	double eff = hEffic[e]->GetBinContent(ptbin);
  // 	double old_value = hUE2D[jp][e]->GetBinContent(xbin);
  // 	double old_err = hUE2D[jp][e]->GetBinError(xbin);
  // 	double corr_value = (double) old_value/eff;
  // 	double corr_err = (double) old_err/eff;
  // 	hUE2D_te[jp][e]->SetBinContent( xbin, corr_value);
  // 	hUE2D_te[jp][e]->SetBinError( xbin, corr_err );
  //     }
  ...
  //     hAddedUE_te[pval][eval]->Add(hUE2D_te[jp][e]);
  //   }
  // }

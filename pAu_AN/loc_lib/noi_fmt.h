...
TH1* noi_fmt (TH1* hg, noiDict _override={{}}, noiDict dict=_tu_fmt__hg_dict()) {
    ...
    if (!dict["LineColor"] && !dict["LineColorAlpha"] && (dict["MarkerColor"] || dict["MarkerColorAlpha"])) {
        ...
        if (dict["LineColorAlpha"]) hg->SetLineColorAlpha(dict("MarkerColorAlpha",1),dict("MarkerColorAlpha",2));
    }
    ...
    if (dict["yAxisRange"]) hg->GetXaxis()->SetRangeUser(dict("yAxisRange",1),dict("yAxisRange",2));

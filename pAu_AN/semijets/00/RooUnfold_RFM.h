...
struct RooUnfold_RFM_Array {
    ...
    TCanvas* plot_fakes(tuOptMap opts={}) {
        TCanvas* canv = new TCanvas(tuUniqueName(), tuUniqueName(), 800, 500 );
        canv->SetLogy();
        gStyle->SetOptTitle(0);
        vector<int> colors { 924, 952, 980, 1008, 1036, 1064, 1092, 1120, 1148, 1176 };
        /* vector<int> colors { 1179, 1210, 1241, 1272, 1303, 1334, 1365, 1396, 1427 }; */
        auto pthat_bins {vector<int>{5,7,9,11,15,25,35,45,55,65}};
        for (int i=0;i<9;++i) { 
            tuOptMap plot_opt {{ "Title","",
                        "xAxisTitle","#it{p}_{T} Fake Jets [GeV/#it{c}]",
                        "yAxisTitle","Normalized # jets",
                        "xAxisTitleSize",15,"yAxisTitleSize",15,
                        "yAxisLabelSize",15, "yAxisTitleOffset",1.8,
                        "xAxisLabelSize",15, "xAxisTitleOffset",1.2,
                        /* "zAxisNdivisions",6, */
                        /* "xAxisNdivisions",6, */
                        "yAxisNdivisions",6,
                        "xAxisRangeLo",0.,"xAxisRangeHi",1.
            }};
            plot_opt += opts;
            plot_opt.add({{"MarkerStyle", kFullCircle, "MarkerColor", colors[i], "FillColor",colors[i],"FillAlpha",0.5}});
            auto h1 = (TH1D*) data[i].fakes->Clone(tuUniqueName());
            if (h1->Integral()>0.) h1->Scale(1./h1->Integral());
            tu_fmt(h1,plot_opt);
            h1->SetTitle(Form("#hat{#it{p}}_{T} #in [%i,%i]",pthat_bins[i],pthat_bins[i+1]));

            /* if (i==5) h1->Draw("P*E3"); */
            h1->Draw((i==0) ? "P*E3" : "P*E3 same");
            /* tuDrawTLatex(Form("#hat{#it{p}}_{T} #in [%i,%i]",pthat_bins[i],pthat_bins[i+1])); */
        }
        canv->BuildLegend();
        return canv;
    }
    TCanvas* plot_misses(tuOptMap opts={}) {
        TCanvas* canv = new TCanvas(tuUniqueName(), tuUniqueName(), 800, 500 );
        canv->SetLogy();
        gStyle->SetOptTitle(0);
        vector<int> colors { 924, 952, 980, 1008, 1036, 1064, 1092, 1120, 1148, 1176 };
        ...
        for (int i=0;i<9;++i) { 
            tuOptMap plot_opt {{ "Title","",
                        "xAxisTitle","#it{p}_{T} Missed Jets [GeV/#it{c}]",
                        "yAxisTitle","Normalized # jets",
                        "xAxisTitleSize",15,"yAxisTitleSize",15,
                        "yAxisLabelSize",15, "yAxisTitleOffset",1.8,
                        "xAxisLabelSize",15, "xAxisTitleOffset",1.2,
            ...
            plot_opt += opts;
            plot_opt.add({{"MarkerStyle", kFullCircle, "MarkerColor", colors[i], "FillColor",colors[i],"FillAlpha",0.5}});
            auto h1 = (TH1D*) data[i].miss->Clone(tuUniqueName());
            if (h1->Integral()>0.) h1->Scale(1./h1->Integral());
            tu_fmt(h1,plot_opt);
            h1->SetTitle(Form("#hat{#it{p}}_{T} #in [%i,%i]",pthat_bins[i],pthat_bins[i+1]));

root -l <<EOF
    ...
    /* tuBinVec bin_zdcx    {{ 4000., 4000., 24,   28000. }}; */
    ...
    pads(0);
    for (int i=0; i<4; ++i) {
        hg_raw[i]->GetYaxis()->SetRangeUser(0., 1.2);
        hg_raw[i]->Draw( i == 0 ? "PE" : "PE same" );
        hg_cor[i]->Draw( "PE same" );
    }
    pads(0)->BuildLegend();

    for (int i=0; i<4; ++i) {
        const char* tag = i==0 ? "Au-going (#eta#in[-1,-0.3]) :" :
                          i==1 ? "mid      (#eta#in[-0.3,0.3] :" :
                          i==2 ? "p-going  (#eta#in[0.3,1])   :" :
                                 "all-#eta (#eta#in[-1,1])    :";

        noiDrawTLatex(Form("Fit dNch/d#eta/d#phi :  %s %6.3f^{#pm%6.4f} + %13.9g^{#pm%10.9g} * zdcX", 
                    tag,
                pvals[i][0], evals[i][0], pvals[i][1], evals[i][1]), 5000., 0.5+(0.05*i), 
                {{"TextColorAlpha",colors[i],1.,"LineStyle",2,"TextSize",23}});
    }
    ...
    msg_tree.write();
    f_save.WriteObject(&pvals, "PU_fit_values");
    f_save.WriteObject(&evals, "PU_fit_errors");
    f_save.Save();
    f_save.Close();

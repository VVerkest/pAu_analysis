root -l <<EOF
    ...
    array<TH1D*, 9> hg_eff;
    array<TH1D*, 9> hg_data1cm;
    array<TH1D*, 9> hg_data3cm;
    ...
    for (int i=0;i<9;++i) {
        ...
        // get the value twice -- one for 1 cm DCA and then again for 3 cm DCA
        set_range(_dca,   1, 5, {tracks}, false);
        set_range(_zdcx, i0_zdcx[i], i1_zdcx[i], {tracks, events}, false);
        auto h_raw1cm  = (TH1D*)  tracks->Projection(_pt);
        h_raw1cm->SetName("h_raw1cm");
        auto h_event = (TH1D*) events->Projection(_zdcx);
        h_raw1cm->Scale(1./h_event->Integral(3,-1));
        hg_data1cm[i] = h_raw1cm;

        set_range(_dca,  11, 15, {tracks});
        auto h_raw3cm  = (TH1D*)  tracks->Projection(_pt);
        h_raw3cm->SetName("h_raw3cm");
        h_raw3cm->Scale(1./h_event->Integral(3,-1));
        hg_data3cm[i] = h_raw3cm;
    ...
    for (int j=0;j<9;++j) { // j for emb
        // 1cm version
        auto hdata = (TH1D*) hg_data1cm[i]->Clone("hg_eff");
        ...
        // 3cm version
        hdata = (TH1D*) hg_data3cm[i]->Clone("hg_eff");
        val0 = hdata->Integral(3,-1);
        div_by_W(hdata,hg_eff[j]);
        val1 = hdata->Integral(3,-1);
        delete hdata ;
        aval[j][i] /= val0/val1;

...
namespace pAuAnalysis {
  ...
  bool UseEvent( TStarJetPicoEventHeader* Header, TStarJetPicoEvent* Event, double vz_cut, double vz, std::string triggerOption ) {
    ...
    if (Header->GetRunId() >= 16142059 && Header->GetRunId() <= 16149001) { return false; std::cout<<"1"; }    //TEMPORARILY SKIPPING THESE RUNS
    else if (Header->GetRunId() == 16135031 || Header->GetRunId() == 16135032) { return false; std::cout<<"2"; }
    else if ( abs(vz) > vz_cut ) { return false; std::cout<<"3"; }

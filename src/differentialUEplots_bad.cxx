...
void FitAndSaveEfficHisto( TFile *effic_File, TH1D *h_Eff, TF1 *effic_Fit, int etaBin, int EAbin, TString dir_name ){

  TCanvas *can = new TCanvas( "can" , "" ,700 ,500 ); 
  TString name;
  int binno = etaBin+1;

  cout<<effic_File<<endl;
    
  // if (EAbin==0 ) { name = "eff_s_bin_1_3_bbc__"; name += binno; name += "_"; name += binno; name += "_eta"; }
  // else if (EAbin==1) {name = "eff_s_bin_7_10_bbc__"; name += binno; name += "_"; name += binno; name += "_eta"; }
    
  // h_Eff = (TH1D*)effic_File->Get(name);

  // name = EAbinName[EAbin] + "EA_effic_etaBin"; name += etaBin;

  // h_Eff->SetAxisRange(0.0,1.3,"Y");
  // h_Eff->SetAxisRange(0.0,15.0,"X");
  // h_Eff->Fit( "eff", "EM" );

  // effic_Fit = h_Eff->GetFunction("eff");
  // effic_Fit->SetName(name);
      
  // h_Eff->Draw();
      
  // TString saveName = "plots/UE/" + dir_name + "/" + name + ".pdf";
...
int main(){
  ...
  for (int i=0; i<nEAbins; ++i) { efficFile[i] = new TFile( efficFileName[i], "READ" ); }

  for (int i=0; i<eta_bins; ++i) {
    for (int j=0; j<nEAbins; ++j){ FitAndSaveEfficHisto( efficFile[i], hEff[i][j], efficFit[i][j], i, j, dirName ); }
  }
  ...
  // void ApplyTrackingEfficiency( TH2D* h_ChgUEpt_te, TH2D *h_ChgUEpt, double eff_offset ) 

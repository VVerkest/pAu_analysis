...
int main ( int argc, const char** argv ) {         // funcions and cuts specified in pAuFunctions.hh
  ...
  for ( int p=0; p<3; ++p ) {
    for ( int e=0; e<3; ++e ) {
      for ( int c=0; c<3; ++c ) {
	name = "hRho" + ptBinName[p] + etaBinName[e] + BackgroundChargeBias[c];	title = "";		hRhoByEta[p][e][c] = new THD( name, title, nEtaBins,0.0,3.0 );
  ...
  while ( Reader.NextEvent() ) {
	...
	for ( int c=0; c<3; ++c ) {	  //  BACKGROUND ESTIMATION 
	  ...
	  hRhoByEta[p][e][c]->Fill( 0.5, eastRho );
	  hRhoByEta[p][e][c]->Fill( 1.5, midRho );
	  hRhoByEta[p][e][c]->Fill( 2.5, westRho );

...
int main ( int argc, const char** argv ) {         // funcions and cuts specified in pAuFunctions.hh
  ...
  bool LoEA = true;		if ( LoEA==true ){ BBCEmin = 8000;    BBCEmax = 16000; }
  ...
  TH1D *hBbcAdcEastSum[nEtaBins];

  for ( int p=0; p<3; ++p ) {

    name = "hBBCEsum" + etaBinName[p];
    title = "BBC ADC East Sum: " + etaBinString[p];
    hBbcAdcEastSum[p] = new TH1D( name, title, 160,0,80000 );
  ...
  while ( Reader.NextEvent() ) {
    ...
    hBbcAdcEastSum[eval]->Fill( header->GetBbcAdcSumEast() );

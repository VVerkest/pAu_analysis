...
int main () {
  ...
  TH1D* hfreq_above2 = (TH1D*) fin->Get("hTrigTowFreq_Eabove2"); //for scaling the weighted energy 1D by the counts to get the average E per tower
  TH1D* hfreq = (TH1D*) fin->Get("hTrigTowFreq");
  TH1D* henergy = (TH1D*) fin->Get("hTrigTowFreq_weighted");
  TH1D* henergy_above2 = (TH1D*) fin->Get("hTrigTowFreq_weighted_Eabove2");
  ...
  fstream file("bad_trigger_towers_pAu2015.list", fstream::in | fstream::out | fstream::trunc);
  if (!file) {
  ...
  file.close();
  cout << "Closed bad_trigger_towers_pAu2015.list" << endl;
  ...
  TFile *fout = new TFile("QA/triggerTowerRemovalHistograms.root","RECREATE");

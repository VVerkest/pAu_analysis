...
int main ( int argc, const char** argv ) {         // funcions and cuts specified in pAuFunctions.hh
  ...
  while ( Reader.NextEvent() ) {
    ...
    for ( int i=0; i<chgParticles.size(); ++i ) {
      ...
      if ( BGeta<etaLowMid && BGeta>etaHiMid ) { chgMidSum+= chgParticles[i].pt() }
      else if ( BGeta<etaLowEast && BGeta>etaHiEast ) { chgEastSum+= chgParticles[i].pt() }
      else if ( BGeta<etaLowWest && BGeta>etaHiWest ) { chgWestSum+= chgParticles[i].pt() }
    ...
    for ( int i=0; i<neuParticles.size(); ++i ) {
      ...
      if ( BGeta<etaLowMid && BGeta>etaHiMid ) { neuMidSum+= neuParticles[i].pt() }
      else if ( BGeta<etaLowEast && BGeta>etaHiEast ) { neuEastSum+= neuParticles[i].pt() }
      else if ( BGeta<etaLowWest && BGeta>etaHiWest ) { neuWestSum+= neuParticles[i].pt() }

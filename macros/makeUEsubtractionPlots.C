...
void makeUEsubtractionPlots(){
  ...
  // TString fileName = "out/UE/pAuHTjetUE.root";track
  ...
  //int nWestProfBins = hWestRhoProfile->GetNbinsX();
   ...
   TFile* outFile = new TFile( "src/UEsubtractionPlots.root", "RECREATE" );

   hEastRhoByBBCEsum->Write();
   hEastRhoProfile->Write();
   hMidRhoByBBCEsum->Write();
   hMidRhoProfile->Write();
   hWestRhoByBBCEsum->Write();
   hWestRhoProfile->Write();

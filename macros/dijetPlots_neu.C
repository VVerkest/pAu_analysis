...
TCanvas * c0 = new TCanvas( "c0" , "" ,0 ,23 ,1280 ,700 );              // CANVAS
  ...
  c1->cd();
  c1->SetLogy();
  TH2D *hscale5  = new TH2D( "hscale5", "Neutral Background Particle #eta by Lead Jet p_{T};#eta;", 40,-1.0,1.0, 10,0.2,1.0 );
  hscale5->SetStats(0);
  hscale5->Draw();
  for ( int i=0; i<nPtBins; ++i ) {
  ...
  for ( int i=0; i<nPtBins; ++i ) {
    ...
    title = "Neutral Underlying Event vs. BBC East Rate - p_{T}^{lead}: " + ptBinString[i];
    ...
    title = "Neutral Underlying Event vs. BBC ADC East Sum - p_{T}^{lead}: " + ptBinString[i];
  ...
  TH2D *hscale_1 = new TH2D( "hscale_1", "Neutral Underlying Event vs. BBC East Rate", 140,0,7000000, 50,0,25 );
  TH2D *hscale_2 = new TH2D( "hscale_2", "Neutral Underlying Event vs. BBC ADC East Sum", 150,0,100000, 50,0,25 );
  ...
  TH2D *hscale3 = new TH2D( "hscale3", "Neutral Underlying Event vs. BBC East Rate", 50,0,25, 100,0.000001,1 );
  TH2D *hscale4 = new TH2D( "hscale4", "Neutral Underlying Event vs. BBC ADC East Sum", 50,0,25, 100,0.000001,1 );
